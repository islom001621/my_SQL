...
Create database Lesson_13_homework;


use Lesson_13_homework

--Tables


create table Employees (EMPLOYEE_ID int, FIRST_NAME varchar(50), LAST_NAME varchar(50),EMAIL 
varchar(255), PHONE_NUMBER varchar(50),HIRE_DATE date,JOB_ID varchar(50),SALARY float,
COMMISSION_PCT float, MANAGER_ID int, DEPARTMENT_ID int)
...
SELECT CONCAT(EMPLOYEE_ID, '-',FIRST_NAME, space(1), LAST_NAME) as employee_info
...
select FIRST_NAME as "Name", len(first_name) as "Len_name"
from Employees
...
select * from SingleOrder
order by IIf(id= 0,1,0);

-- task 8

select coalesce(ssn,passportid,itin)as CoalesceColumn
from person;
...
select  string_agg(String,' ') as Fullanswer
from DMLTable;
...
SELECT jobdescription, 
       MIN(MissionCount) as least_experienced_id,
       MAX(MissionCount) as most_experienced_id
FROM Personal
...
WITH Tally(n) AS (
    SELECT TOP (LEN('tf56sd#%OqH')) ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) 
...
SELECT
    ...
    STRING_AGG(CASE WHEN ch COLLATE Latin1_General_BIN NOT LIKE '[A-Za-z0-9]' THEN ch END, '') AS Others
FROM Chars;
...
------------------no answer cuz of the misunderstanding---------------------------------------
