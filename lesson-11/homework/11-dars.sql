...
select Departments.DepartmentName,Employees.Name as TopEmployee,e.MaxSalalry from Departments
inner join (select Employees.DepartmentID,
            max(Salary) as MaxSalalry from Employees
            group by Employees.DepartmentID ) as E
on Departments.DepartmentID=E.DepartmentID 
inner join Employees
on Employees.DepartmentID=E.DepartmentID and Employees.Salary=e.MaxSalalry
...
where year (Orders.OrderDate)= 2023 and Customers.Country='USA';
...
left join (
 ...
 from orders
	...
	on c.customerid = latest.customerid
left join orders o 
    on o.customerid = latest.customerid and o.orderdate = latest.last_order_date;
...
select 
    p.productid,
    p.productname,
    sum(p.stockquantity * p.price) as Total_sales
from products p
...
select e.employeeid, e.name, e.hiredate, d.departmentname
from employees e
join departments d On e.departmentid = d.departmentid
...
select o.orderid, o.orderdate, c.customerid, c.firstname
from  orders o
...
select p.productid, p.productname,  P.category,  s.saleamount,  s.saledate
from  products p
join sales s on p.productid = s.productid
where s.saleamount > 350;
...
select o.orderid, o.orderdate, o.totalamount, c.firstname, c.city
from orders o
join customers c on o.customerid = c.customerid
