...
create database Lesson_14_homework;

use Lesson_14_homework;
...
PHONE_NUMBER varchar(50),HIRE_DATE date,JOB_ID varchar(50),SALARY float,COMMISSION_PCT float, MANAGER_ID int, DEPARTMENT_ID int)
...
select value from 
string_split (( select string_agg(vals,'.')from Splitter),'.');
...
JOIN (
    ...
    FROM Activity
    GROUP BY player_id
) B ON A.player_id = B.player_id AND A.event_date = B.first_date;
